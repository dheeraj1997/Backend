...
class AdmissionService {
    generateAdmissionForm(html, filename) {
        let deffered = q.defer();
        let ts = Date.now().toString();
        let clientDir = path.join(__dirname, '../public/student/admission');
    ...
    generateIdentityCard(html, filename) {
        let deffered = q.defer();
        let ts = Date.now().toString();
        let clientDir = path.join(__dirname, '../public/student/identity');
        let pdfPath = path.join(clientDir, filename + '_' + ts + '.pdf');
...
exports.getInstance = function () {
    if (!instance) instance = new AdmissionService();
    return instance;
};